...
def Jv2(bs):
    ...
    def int_at_fixed_Ep(Ep):
        ...
        return np.trapz(np.vectorize(lambda e: integrand(e,Ep))(E),E)
    ...
    return np.trapz(np.vectorize(int_at_fixed_Ep)(Ep),Ep)   
...
def current(bs,method="integral"):
    method={"orig": J, "v2": Jv2, "d": J_d, "integral": J, "meanfield": J_trimean, "maxfield": J_tri}[method]
    return method(bs)
